...
`;
    ...
    const q1 = `SELECT * FROM vehicle_models WHERE model = ?`;
    const [rows] = await pool.query(q1, [model]);
    if (rows.length === 0) {
    ...
    async getMaintainenceVehicles() {
        const query = 'SELECT v.vehicle_id, v.model, v.year, v.color,v.rides,v.license_plate,v.price_per_day, m.brand,m.vehicle_type FROM vehicles AS v, vehicle_models AS m WHERE v.model = m.model AND v.status = "maintenance"';
        const [rows] = await pool.execute(query);
        return rows;
    },
    async getVehiclesWithModelBrand() {
        const query = 'SELECT v.vehicle_id, v.model, v.year, v.color,v.rides,v.license_plate,v.price_per_day, m.brand,m.vehicle_type FROM vehicles AS v, vehicle_models AS m WHERE v.model = m.model AND v.status = "avail"';
        const [rows] = await pool.execute(query);
        return rows;
    },
    ...
    async createVehicle(model, year, color, rides, license_plate, price_per_day, status) {
        const query = 'INSERT INTO vehicles (model,year,color,rides,license_plate, price_per_day,status) VALUES (?,?,?,?,?,?,?)';
        await pool.execute(query, [model, year, color, rides, license_plate, price_per_day, status]);
    },
    async updateVehicle( price_per_day, status, id) {
        const query = 'UPDATE vehicles SET  price_per_day = ?, status = ? WHERE vehicle_id = ?';
        const [rows]= await pool.execute(query, [ price_per_day, status, id]);
        return rows;
    },
