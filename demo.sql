...
CREATE PROCEDURE process_transaction (
    ...
    IN e_maintenance_id INT DEFAULT NULL
)
BEGIN
    INSERT INTO transactions (customer_id, amount, payment_method, transaction_type)
    VALUES (e_customer_id, e_amount, e_payment_method, e_transaction_type);
...
-- is coupon valid?
DELIMITER $$
CREATE FUNCTION fn_check_coupon_validity(p_coupon_code VARCHAR(20))
RETURNS TINYINT
DETERMINISTIC
BEGIN
    DECLARE v_valid_from DATE;
    DECLARE v_valid_until DATE;
    DECLARE v_current_date DATE;
    DECLARE v_coupon_exists TINYINT DEFAULT 0;
    ...
    IF v_coupon_exists = 0 THEN
        RETURN 0; -- Coupon doesn't exist
    END IF;

    -- Get coupon validity dates
...
AFTER INSERT ON booking_payment
FOR EACH ROW
BEGIN
    UPDATE bookings
